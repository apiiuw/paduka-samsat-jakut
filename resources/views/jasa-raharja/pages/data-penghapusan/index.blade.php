...
<div class="p-4 sm:ml-64 bg-[#ECF3F7] min-h-screen">
   <div class="p-4 mt-24">
      ...
      <div class="bg-white p-4 rounded-xl shadow-md">
         <p class="font-semibold text-lg text-[#373A3C]">Filter Data Penghapusan</p>
         <hr class="bg-[#E8EEF2] h-[2px] mt-4 mb-8">
         <form action="{{ route('data-penghapusan.index') }}" method="GET" class="flex justify-between flex-wrap gap-2">
            <!-- Tahun -->
      ...
      <div class="mx-auto bg-white p-6 rounded-xl shadow-md mt-8">
         ...
         <div class="flex items-center gap-2 mb-4 text-sm">
            <!-- Form Pencarian -->
            <form action="{{ route('data-penghapusan.index') }}" method="GET" class="flex w-full">
               <!-- Input Pencarian -->
               <input type="text" name="search" placeholder="Ketikkan Nama atau Email Pemilik..." value="{{ request('search') }}" class="flex-grow border text-sm rounded px-4 py-2 mr-2" />
            ...
            <table class="min-w-[1600px] border text-sm text-left">
                <thead class="bg-[#373D53] text-white text-center">
                    <tr>
                        ...
                        <th class="border px-2 py-1">Tanggal Pengajuan</th>
                        <th class="border px-2 py-1">Nama Pemilik</th>
                        <th class="border px-2 py-1">Alamat Pemilik</th>
                        <th class="border px-2 py-1">NIK/TDP/NIB/Kitas/Kitab</th>
                        <th class="border px-2 py-1">No. Telp/WA</th>
                        <th class="border px-2 py-1">Email</th>
                        <th class="border px-2 py-1">NRKB</th>
                        <th class="border px-2 py-1">Merek</th>
                        <th class="border px-2 py-1">Tipe</th>
                        <th class="border px-2 py-1">Jenis</th>
                        <th class="border px-2 py-1">File PDF</th>
                    </tr>
                </thead>
                <tbody class="bg-white">
                    ...
                    @foreach ($dataLaporan as $item)
                        <tr class="border-t text-center">
                            <td class="border px-2 py-3">{{ $no++ }}</td> <!-- Nomor urut -->
                            <td class="border px-2 py-3">{{ $item->tanggal_form }}</td>
                            <td class="border px-2 py-3">{{ $item->nama_pemilik }}</td>
                            <td class="border px-2 py-3">{{ $item->alamat_sesuai_identitas }}</td>
                            <td class="border px-2 py-3">{{ $item->nik_tdp_nib_kitas_kitab }}</td>
                            <td class="border px-2 py-3">{{ $item->no_telp }}</td>
                            <td class="border px-2 py-3">{{ $item->email }}</td>
                            <td class="border px-2 py-3">{{ $item->nrkb_kendaraan }}</td>
                            <td class="border px-2 py-3">{{ $item->merek_kendaraan }}</td>
                            <td class="border px-2 py-3">{{ $item->tipe_kendaraan }}</td>
                            <td class="border px-2 py-3">{{ $item->jenis_kendaraan }}</td>
                            <td class="border px-2 py-3">
                                @if ($item->file_pdf)
                                    <!-- Menggunakan URL yang dapat diakses dari public/storage -->
                                    <a href="{{ asset('storage/' . $item->file_pdf) }}" target="_blank" class="bg-green-500 text-white px-4 py-2 rounded">
                                        Lihat PDF
                                    </a>
                                @else

...
<div class="p-4 sm:ml-64 bg-[#ECF3F7]" x-data="formHandler()">
    <div class="p-4 mt-24">
        <div class="bg-white shadow rounded-lg p-6 text-sm">
            ...
            <form class="space-y-4" method="POST" action="{{ route('surveyor.hasil-survei.store.tersangka') }}" enctype="multipart/form-data" onsubmit="return confirm('Yakin menyimpan data?')">
                @csrf
                ...
                <div>
                    ...
                    <select class="mt-1 text-sm w-full border border-gray-300 rounded px-3 py-2 bg-gray-100 text-gray-700 cursor-not-allowed"
                        ...
                        <option value="Roda 2" {{ $data->jenis_kendaraan_tersangka == 'Roda 2' ? 'selected' : '' }}>Roda 2</option>
                        <option value="Roda 3" {{ $data->jenis_kendaraan_tersangka == 'Roda 3' ? 'selected' : '' }}>Roda 3</option>
                        <option value="Roda 4" {{ $data->jenis_kendaraan_tersangka == 'Roda 4' ? 'selected' : '' }}>Roda 4</option>
                        <option value="Diatas Roda 4" {{ $data->jenis_kendaraan_tersangka == 'Diatas Roda 4' ? 'selected' : '' }}>Diatas Roda 4</option>
                    </select>
                    <input type="hidden" name="jenis_kbm" value="{{ $data->jenis_kendaraan_tersangka }}">
                </div>
